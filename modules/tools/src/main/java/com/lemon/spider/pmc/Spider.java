...
public class Spider {
    ...
    public static int PMC_Start = 4500000;
    public static int PMC_Stop =  4510000;
    public static String WWWROOTDir = "E:\\pdfs\\";
...
//            context = new FileSystemXmlApplicationContext("classpath*:applicationContext.xml");
            if(left >= 0 && left < threads) {
                ParserPaper main = new ParserPaper(threads, left);
                main.start();
                ThreadSafeSleep.sleep(4000);
            } else {
                for (int i = 0; i < threads; i++) {
                    ParserPaper main = new ParserPaper(threads, i);
                    main.start();
                    ThreadSafeSleep.sleep(4000);
                }
            }
