...
public class LemonModSQLFunction extends StandardSQLFunction {
	public LemonModSQLFunction(String name) {
		super(name, org.hibernate.type.IntegerType.INSTANCE);
	}

	public LemonModSQLFunction(String name, Type typeValue) {
	...
	public String render(Type type, List args, SessionFactoryImplementor arg2) {
		if (args.size() != 2) {
			throw new IllegalArgumentException("the function must be passed 2 arguments, such as a%b");
		}
		...
		if (type instanceof org.hibernate.type.IntegerType || type instanceof org.hibernate.type.BigIntegerType || type instanceof org.hibernate.type.ShortType) {
			...
			buffer.append(" % ").append(args.get(1)).append(" ");
